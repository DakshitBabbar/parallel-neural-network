...
//=========================trial 1=============================//
  struct fann_neuron *neurons = last_layer_begin; 

  float *ann_MSE = &(ann->MSE_value);
  fann_type *ann_bit_fail_limit = &(ann->bit_fail_limit);
  unsigned int *ann_num_bit_fail = &(ann->num_bit_fail);

  enum fann_errorfunc_enum *ann_train_error_function = &(ann->train_error_function);

  //printf("num_MSE lmao= %u\n", ann->num_MSE);
  unsigned int *ann_num_MSE = &(ann->num_MSE);
  unsigned int num_neurons = ann->num_output;

  compute_MSE_parallel(num_neurons, neurons, desired_output, error_it, ann_MSE, ann_bit_fail_limit, ann_num_bit_fail, ann_train_error_function, ann_num_MSE);
//=============================================================//
...
void fann_backpropagate_MSE(struct fann *ann) {
  ...
  fann_type *slope_begin = ann->train_slopes;
  //=======================================================================//
  ...
  int l = 3;
  bool f = false;
  for (layer_it = last_layer - 1; layer_it >= second_layer; --layer_it) { //remove '=' sign if revert
    last_neuron = layer_it->last_neuron;

    //===============================trial 1==============================//
    struct fann_neuron *prev_neurons = (layer_it - 1)->first_neuron;

    fann_type *prev_errors = error_begin + ((layer_it - 1)->first_neuron - first_neuron);
    fann_type *prev_slopes = slope_begin + (layer_it->first_neuron->first_con);
    fann_type *errors = error_begin + (layer_it->first_neuron - first_neuron);
    fann_type *my_weights = ann->weights + layer_it->first_neuron->first_con;

    unsigned int num_prev_neurons = ((layer_it - 1)->last_neuron) - ((layer_it - 1)->first_neuron);
    unsigned int num_neurons = ((layer_it)->last_neuron) - ((layer_it)->first_neuron);
    unsigned int num_weights = (num_neurons-1)*num_prev_neurons;

    //printf("move back for %d\n", l);
    if(layer_it == second_layer){
      f = true;
    }
    backpropagate_MSE_parallel(prev_neurons, prev_errors, errors, my_weights, num_prev_neurons, num_neurons, num_weights, prev_slopes, f);
    l--;
    //====================================================================//
...
void fann_update_slopes_batch(struct fann *ann, struct fann_layer *layer_begin,
  ...
  for (; layer_begin <= layer_end; layer_begin++) {
    ...
    //===========================trial 1==========================//
    fann_type *prev_slopes = slope_begin + layer_begin->first_neuron->first_con;
    struct fann_neuron *prev_neurons = (layer_begin - 1)->first_neuron;
    fann_type *my_errors = error_begin + ((layer_begin->first_neuron) - first_neuron);

    unsigned int num_neurons = ((layer_begin)->last_neuron) - ((layer_begin)->first_neuron);
    unsigned int num_prev_neurons = ((layer_begin-1)->last_neuron) - ((layer_begin-1)->first_neuron);
    unsigned int num_slopes = num_prev_neurons*(num_neurons - 1);

    update_slopes_parallel(prev_slopes, prev_neurons, my_errors, num_slopes, num_prev_neurons, num_neurons);
    //============================================================//
